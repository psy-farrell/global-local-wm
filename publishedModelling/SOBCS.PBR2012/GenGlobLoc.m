...
for i = 1:E.setsize+1
	if i == 1, splegtext{i} = ['Global distr: ']; %#ok<AGROW>
	else
		if E.distvar == 3
			splegtext{i} = ['Exceptn posn: ', mat2str(i-1),'-',mat2str(i)]; %#ok<*AGROW>
		else
...
for dsim = 1:size(E.dsim,2)  %similarity between distractors
		...
		for globloc = 1:E.maxglbcond   %loops through global bursts between all spoken/all typed
			for exceptpos = 1:ncond   % loops through trials for each control/exception condition.
				...
				for trial = 1:E.trialnum
					...
					%saving data
					fprintf (fid, '%3.0f %2.0f %1.2f %1.2f %1.2f %1.0f %1.0f %1.0f  %1.0f %1.0f %4.2f %4.2f %4.2f %4.2f %4.2f %4.2f %2.1f %3.2f %3.2f %3.2f   %3.2f %3.2f %3.2f %1.2f    %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f   %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f %1.0f \n', ...
						id, model, E.dsim(dsim), E.opduration(od), E.freetime(ot), E.opnum(on), E.setsize, E.simcond, ...
						1, P.recallset, P.threshold, P.nsubo, P.c, P.rate, P.removalrate, P.tau, P.gain, 0, 0, 0, ...
	...
	fid2 = fopen(['gl', mat2str(experiment-9), 'sobpred.dat'], 'w');
