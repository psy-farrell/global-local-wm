...
dat = load(['TBRS.GlobLocGrid9-' num2str(rehType) '.out']);
...
mask = meanAcc > 0.3 & meanAcc < 0.9; %& abs(D1(:,4)-D1(:,6))<0.05;
...
filen = ['TBRSspaceE1-' num2str(rehType) 'aaa.pdf'];
...
filen = ['TBRSpredE1-' num2str(rehType) 'aaa.pdf'];
export_fig(filen, '-nocrop');
