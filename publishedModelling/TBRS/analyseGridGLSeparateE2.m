...
dat = load(['TBRS.GlobLocGrid11-' num2str(rehType) '.out']);
...
%SPmaxm = SPdiff(:,1)-SPdiff(:,5);
%SPmaxm = SPdiff(:,2)-SPdiff(:,4);
...
% line(-0.0575+[-0.0352 0.0352],[-0.0729 -0.0729]);
% line([-0.0575 -0.0575],-0.0729+[-0.2774 0.2774]);
xlabel('Locality');
ylabel('Asymmetry');
...
filen = ['TBRSspaceE2-' num2str(rehType) '.pdf']; %#ok<NASGU>
export_fig(filen, '-nocrop')
...
filen = ['TBRSpredE2-' num2str(rehType) '.pdf'];
export_fig(filen)